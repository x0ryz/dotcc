...
const Projects = ({ className }) => {
  return (
    <Card
      className={`${className} flex flex-col gap-0 p-0 overflow-hidden bg-[#ff99c8]`}
    >
      ...
      <div className="flex flex-col h-full divide-y-2 divide-black bg-white">
        {PROJECTS.map((proj, idx) => (
          ...
          <div key={idx} className="flex-1 flex flex-row h-full min-h-0">
            ...
            <div className="flex-1 p-3 flex flex-col justify-center gap-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                {/* ЗМІНА: прибрав truncate, додав leading-tight для переносу тексту */}
              ...
              {/* ЗМІНА:
                  1. text-[10px] -> text-xs (більший шрифт)
                  2. line-clamp-2 -> line-clamp-4 (більше тексту влазить)
              */}
              <p className="text-xs text-gray-600 line-clamp-4 md:line-clamp-3 leading-tight">
                {proj.desc}
              </p>
              ...
              <div className="flex flex-wrap gap-1 mt-auto pt-2">
                {proj.tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    className="text-[9px] border border-black px-1 bg-gray-50 whitespace-nowrap"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div className="w-[50px] sm:w-[60px] flex flex-col border-l-2 border-black shrink-0">
              ...
              {proj.github ? (
                <a
                  ...
                  className="flex-1 flex items-center justify-center hover:bg-black hover:text-white transition-colorsyb cursor-pointer bg-gray-50"
                  title="Code"
                >
                  <Github size={18} />
                </a>
